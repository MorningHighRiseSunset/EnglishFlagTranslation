...
const SITE_MAIN_TARGET_RAW = process.env.SITE_MAIN_TARGET || 'en';
...
exports.handler = async function(event) {
  ...
  try {
    ...
    // default target: prefer SITE_MAIN_TARGET (set per Netlify site), otherwise fall back to English
    let targetCode = SITE_MAIN_TARGET || 'en';
     if (userTarget) {
      ...
      if (!sourceCode) {
        try {
          ...
          if (detected) {
            ...
            // English site: if detected source is English -> translate to Spanish
            // Otherwise translate to English (site main target)
            if (!userTarget) {
              if (sourceCode === 'en') targetCode = 'es';
              else targetCode = 'en';
            }
